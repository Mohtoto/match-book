...
5.  **Migration**: 
    -   Run `pnpm db:generate` after schema changes; commit `drizzle/*.sql` and `drizzle/meta/_journal.json`.
    -   Kit users apply with `pnpm db:migrate` (not on `pnpm dev`).
    -   Use `pnpm db:push` only for solo prototyping — **not** for kit releases.
    -   Full workflow: Cursor command `db-migrate` (`.cursor/commands/db-migrate.md`).
