...
- **Migrations**: Use `pnpm db:generate` + commit `drizzle/` for kit releases. Users run `pnpm db:migrate` (auto on `pnpm dev`). `db:push` is prototyping only. See `.cursor/commands/db-migrate.md`.
