...
4. **Optional SDK clients**: Use `getStripe()`, `getDodoClient()`, `getPolar()`, and `getPaddleClient()` inside checkout/billing routes — never instantiate payment SDKs at module load. See `env-handler/reference.md`.
