...
Ensure `.env.local` has AWS credentials (placeholders in `.env.example`):
...
S3 is optional at build time — use `getS3Client()` from `@/lib/s3/client` at runtime (throws if `AWS_REGION` is unset when uploads run).
