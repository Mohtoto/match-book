...
- **Import**: `import { getStripe } from "@/lib/stripe";` then call `getStripe()` inside the handler.
- **Optional service**: `getStripe()` throws at runtime if `STRIPE_SECRET_KEY` is unset. The app still builds without Stripe configured.
